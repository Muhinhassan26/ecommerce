...
router = APIRouter(prefix="/products", tags=["Products - User"])


@router.get("/{product_id}", response_model=ProductResponse)
async def get_product_by_id(
    product_id: int,
    product_service: Annotated[ProductUserService, Depends()],
) -> Any:
    return await product_service.get_product_by_id(product_id=product_id)


@router.get("/paginated/", response_model=PaginatedResponse[ProductResponse])
